#!/usr/bin/env python3
"""
Sync botanical ingredient gallery images to Webflow using Google Drive sharing URLs.

This script:
1. Reads the botanical_image_matches.json
...
Usage:
    python sync_botanical_images_gdrive.py --dry-run    # Show what would be updated
    python sync_botanical_images_gdrive.py --live       # Actually update Webflow
"""
...
MATCHES_FILE = PROJECT_DIR / "botanical_image_matches.json"
...
def load_matches() -> List[Dict]:
    """Load the botanical image matches."""
    if not MATCHES_FILE.exists():
        print(f"Error: Matches file not found: {MATCHES_FILE}")
        print("Run match_botanical_images.py first.")
        sys.exit(1)
...
def sync_gallery_to_webflow(matches: List[Dict], dry_run: bool = True):
    ...
    print()
    print("=" * 70)
    print("SYNCING BOTANICAL GALLERY IMAGES TO WEBFLOW (Google Drive)")
    print("=" * 70)
    ...
    for i, match in enumerate(matches, 1):
        ...
        if i <= 30 or i % 200 == 0:
            ...
            print(f"    Folders: {', '.join(match.get('matched_folders', [])[:3])}")
...
def main():
    print("=" * 70)
    print("BOTANICAL GALLERY IMAGES → WEBFLOW (Google Drive)")
    print("=" * 70)
    print()
    ...
    print()
    print("Quick start:")
    print("  1. Dry run:     python sync_botanical_images_gdrive.py --dry-run")
    print("  2. Live update: python sync_botanical_images_gdrive.py --live")
