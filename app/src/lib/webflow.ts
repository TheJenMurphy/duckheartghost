...
// Parse comma-separated string into pill array (no tooltips from CMS)
...
// Parse price tier from price number
...
// Parse price per oz from price and size
function getPricePerOz(price: number, size: string): string {
  if (!size || size === "N/A") return "";
  // Try to extract oz or g from size string e.g. "0.14 oz" or "30ml"
...
export async function getProductBySlug(slug: string) {
  // Fetch all items and find by slug (Webflow API doesn't support slug filtering)
  let offset = 0;
  const limit = 100;
...
export async function getFirstPublishedProduct() {
  ...
  const data = await res.json();
  const items = data.items || [];
  // Find first published (not draft, not archived) item with affiliate URL
...
function mapWebflowToProduct(f: any) {
  ...
  // Parse safety CSV into cert + regulatory pills
  ...
  // Parse suitability CSV into skinTypes + personas + notFor
  ...
  return {
    ...
    sections: {
      ...
      is: {
        ...
        allIngredients: stripHtml(f["ingredients-2"]).split(",").map(s => s.trim()).filter(Boolean).map(label => ({
          label,
