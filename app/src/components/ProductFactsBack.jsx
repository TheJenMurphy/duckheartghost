...
function SectionContent({ sectionKey, data }) {
  ...
  if (sectionKey === "is") {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {data.description && (
          <div style={{ maxHeight: 100, overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
            <p style={{ fontFamily: "'Outfit', sans-serif", fontSize: "1rem", color: "rgba(255,255,255,.7)", lineHeight: 1.5, margin: 0 }}>
              {data.description}
            </p>
          </div>
        )}
